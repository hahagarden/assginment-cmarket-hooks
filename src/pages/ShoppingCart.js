...
export default function ShoppingCart({ items, cartItems, setCartItems }) {
  ...
  const handleQuantityChange = (quantity, itemId) => {
    const copiedCartItems = cartItems.slice();
    copiedCartItems.find((item) => item.itemId === itemId).quantity = quantity;
  ...
  const handleDelete = (itemId) => {
    ...
    const copiedCartItems = cartItems.slice();
    copiedCartItems.splice(
      copiedCartItems.findIndex((item) => item.itemId === itemId),
      1
    );
    setCartItems(copiedCartItems);
  };
